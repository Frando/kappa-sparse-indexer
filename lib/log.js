...
class Log {
  ...
  flush (cb) {
    ...
    this._lock(release => {
      ...
      function add (value) {
        value = self._valueEncoding.encode(value)
        const seq = ++head
  ...
  get (seq, cb) {
    const opts = { valueEncoding: this._valueEncoding }
    this.db.get(Buffer.concat([LOG, encodeInt(seq)]), opts, (err, value) => {
...
module.exports = class KeyedLog {
  constructor (db, opts = {}) {
    ...
    if (opts.loadValue) {
      this._transform.push(opts.loadValue)
    }
    if (opts.transform) {
      this._transform.push(opts.transform)
    }
  }
  ...
  keyheads (cb) {
    const heads = {}
    for (let key of this._keys.list()) {
  ...
  createReadStream (opts = {}) {
    const self = this
    const stream = this._log.createReadStream(opts).pipe(new Transform({
      objectMode: true,
      transform (row, _enc, next) {
        const { key, seq } = self._decodeValue(row.value)
        const lseq = row.seq
        this.cursor = lseq
        self._onread({ key, seq, lseq }, opts, message => {
          if (message !== null) this.push(message)
          next()
        })
      }
    }))
    stream.cursor = opts.start || 0
    return stream
  }
  ...
  _onread (message, opts, next) {
    ...
    pipeline(message, [...this._transform, opts.transform], next)
  }
