...
function Lock () {
  ...
  function release () {
    if (!queue.length) locked = false
    else {
      queue.shift()(release)
      // process.nextTick(queue.shift(), release)
    }
  }

  function acquire (cb) {
    if (!locked) return cb(release)
    else queue.push(cb)
  }
}
...
module.exports = class MaterializedFeed {
  ...
  append (key, seq) {
    if (seq == null) console.log('append', seq, new Error().stack)
  ...
  add (key, seq, cb) {
    this._log.lookup(key, seq, (err, has) => {
