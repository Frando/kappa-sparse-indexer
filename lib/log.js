...
const { transformPipeline, maybeCallback, loggingMutex } = require('./util')
...
class Log {
  constructor (db, opts = {}) {
    this.db = db
    this._queue = []
    this._lock = loggingMutex(mutex(), 'Log')
...
module.exports = class KeyedLog {
  constructor (db, opts = {}) {
    ...
    this._opts = opts
    this._queue = []
    this._lock = loggingMutex(mutex(), 'KeyedLog')
    this._opened = false
  ...
  sync (cb) {
    this._lock(release => release(cb), 'sync')
  }
