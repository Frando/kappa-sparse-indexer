...
class Log {
  ...
  createReadStream (opts) {
    let { start = 0, end = Infinity, limit } = opts
    // the log starts at 1, so negative numbers can never occur.
    if (!start || start < 0) start = 0
    if (typeof end === 'undefined' && limit) end = start + limit
...
module.exports = class KeyedLog {
  ...
  createReadStream (opts = {}) {
    const self = this
    return this._log.createReadStream(opts).pipe(new Transform({
      objectMode: true,
      transform (node, enc, next) {
        const { key, seq } = self._decodeValue(node.value)
        const lseq = node.seq
