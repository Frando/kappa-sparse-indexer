...
module.exports = class KeyedLog {
  ...
  createReadStream (opts = {}) {
    ...
    const transform = new Transform({
      ...
      transform (row, _enc, next) {
        // decoded = { key, seq }
        const decoded = self._decodeValue(row.value)
        decoded.lseq = row.seq
        this.cursor = row.seq
        self._onread(decoded, opts, message => {
  ...
  _onread (message, opts, next) {
    ...
    transformPipeline(message, [...this._transform, opts.transform], next)
  }
