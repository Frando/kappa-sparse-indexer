const { Transform } = require('stream')
const sub = require('subleveldown')
const collect = require('stream-collector')
const mutex = require('mutexify')

const LOG = 'l/'
const REV = 'r/'
const HEAD = 'head'

// A simple append-only log on a leveldb.
// Supports reverse lookup (value to seq)
class Log {
  constructor (db) {
    this.db = db
    this._queue = []
    this._lock = mutex()
    this._head = undefined
  }

  get length () {
    return this._head
  }

  open (cb) {
    this.head((err, head) => {
      if (err) return cb(err)
      this._head = head
    })
  }

  append (val) {
    this._queue.push(val)
  }

  appendAndFlush (value, cb) {
    this.append(value)
    this.flush(() => {
      this.lookup(value, cb)
    })
  }

  flush (cb) {
    if (!this._queue.length) return cb()
    this._lock(release => {
      const queue = this._queue
      this._queue = []
      this._filterQueue(queue, queue => {
        this.head((err, lasthead) => {
          if (err) return release(cb, err)
          let head = lasthead
          const ops = []
          for (const value of queue) {
            let seq = ++head
            let strseq = encodeInt(seq)
            ops.push({ type: 'put', key: LOG + strseq, value: value })
            ops.push({ type: 'put', key: REV + value, value: seq })
          }
          if (!ops.length) return release(cb)
          ops.push({ type: 'put', key: HEAD, value: head })
          this.db.batch(ops, (err) => {
            this._head = head
            release(cb, err)
          })
        })
      })
    })
  }

  _filterQueue (queue, next) {
    let pending = queue.length
    let filtered = []
    queue.forEach((value, i) => {
      this.lookup(value, (_err, seq) => {
        if (seq === null) filtered.push(value)
        if (--pending === 0) next(filtered)
      })
    })
  }

  createReadStream (opts) {
    let { start, end } = opts
    // the log starts at 1, so negative numbers can never occur.
    if (start < 0) start = 0
    start = start ? encodeInt(start) : ''
    end = end ? encodeInt(end) : '\uFFFF'
    const lvlopts = {
      gte: LOG + start,
      lte: LOG + end
    }
    const transform = new Transform({
      objectMode: true,
      transform (row, enc, next) {
        const seq = row.key.split('/')[1]
        this.push({ seq: decodeInt(seq), value: row.value })
        next()
      }
    })
    return this.db.createReadStream(lvlopts).pipe(transform)
  }

  get (seq, cb) {
    this.db.get(LOG + encodeInt(seq), (err, value) => {
      if (err) return cb(err)
      return cb(null, value)
    })
  }

  lookup (value, cb) {
    this.db.get(REV + value, (err, seq) => {
      if (err) {}
      cb(null, seq ? Number(seq) : null)
    })
  }

  head (cb) {
    this.db.get(HEAD, (err, value) => {
      if (err && err.type !== 'NotFoundError') return cb(err)
      if (err) value = 0
      else value = Number(value)
      cb(null, value)
    })
  }
}

function encodeInt (seq) {
  if (seq === Infinity) return Buffer.alloc(4, 0xff)
  const buf = Buffer.alloc(4)
  buf.writeUInt32BE(seq)
  return buf
}

function decodeInt (str) {
  const buf = Buffer.from(str)
  return buf.readUInt32BE()
}

class Heads {
  constructor (db) {
    this.db = db
  }

  get (key, cb) {
    this.db.get(key, (err, seq) => {
      if (err) seq = -1
      else seq = Number(seq)
      cb(null, seq)
    })
  }

  all (cb) {
    collect(this.db.createReadStream(), (err, res) => {
      if (err) return cb(err)
      res = res.reduce((agg, row) => {
        agg[row.key] = Number(row.value)
        return agg
      }, {})
      cb(null, res)
    })
  }

  put (key, seq, cb) {
    this.get(key, (err, oldseq) => {
      if (err) return cb(err)
      if (seq > oldseq) this.db.put(key, seq, cb)
      else cb()
    })
  }

  batch (heads, cb) {
    let pending = Object.keys(heads).length
    for (const [key, seq] of Object.entries(heads)) {
      this.put(key, seq, () => (--pending === 0 && cb()))
    }
  }
}

module.exports = class MaterializedFeed {
  constructor (db, name) {
    // this._db = db
    this._name = name
    this._log = new Log(sub(db, 'l'))
    this._heads = new Heads(sub(db, 'h'))
    this._queue = []
    this._lock = mutex()
  }

  open (cb) {
    this._log.open(cb)
  }

  keyhead (key, cb) {
    this._heads.get(key, cb)
  }

  keyheads (cb) {
    this._heads.all(cb)
  }

  head (cb) {
    this._log.head(cb)
  }

  get length () {
    return this._log.length
  }

  createReadStream (start, end) {
    const transform = new Transform({
      objectMode: true,
      transform (row, _enc, next) {
        const [key, seq] = row.value.split('@')
        const lseq = row.seq
        this.push({ key, seq: Number(seq), lseq })
        next()
      }
    })
    const rs = this._log.createReadStream({ start, end })
    return rs.pipe(transform)
  }

  read (start, end, cb) {
    collect(this.createReadStream(start, end), cb)
  }

  append (key, seq) {
    if (Array.isArray(key)) {
      key.forEach(([key, seq]) => this.append(key, seq))
      return
    }

    this._queue.push({ key, seq })
  }

  appendAndFlush (rows, cb) {
    this.append(rows)
    this.flush(cb)
  }

  has (key, seq, cb) {
    return this._log.lookup(keyseq(key, seq), (err, lseq) => {
      cb(err, !!lseq)
    })
  }

  keyseqToLseq (key, seq, cb) {
    return this._log.lookup(keyseq(key, seq), cb)
  }

  lseqToKeyseq (lseq, cb) {
    this._log.get(lseq, (err, keyseq) => {
      if (err) return cb(err)
      let [key, seq] = keyseq.split('@')
      seq = Number(seq)
      cb(null, { key, seq })
    })
  }

  flush (cb = noop) {
    if (!this._queue.length) return cb()
    this._lock(release => {
      const queue = this._queue
      this._queue = []

      const heads = {}
      for (const { key, seq } of queue) {
        const fid = keyseq(key, seq)
        this._log.append(fid)
        if (!heads[key] || seq > heads[key]) heads[key] = seq
      }
      this._log.flush(() => {
        this._heads.batch(heads, () => {
          release(cb)
        })
      })
    })
  }
}

function keyseq (key, seq) {
  key = hex(key)
  return key + '@' + seq
}

function hex (key) {
  if (Buffer.isBuffer(key)) return key.toString('hex')
  return key
}

function noop () {}
